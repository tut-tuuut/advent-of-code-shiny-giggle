...
$minuteCandidate = 0;
$candidateValue = 0;
foreach ($formattedPlanning as $guardId => $hour) {
    list($mostAsleepMinute,$value) = getMaxValueAndKey($hour);
    if ($value > $candidateValue) {
        $guardCandidate = $guardId;
        $minuteCandidate = $mostAsleepMinute;
        $candidateValue = $value;
        say ("guard $guardId has slept $value times on minute $mostAsleepMinute ...");
    } 
}
