...
class AsteroidField:
    def __init__(self, strMap):
        ...
        print(
            f"{len(self.asteroids)} asteroids in this {self.width}x{self.height} field..."
        )
    ...
    def parseLinesOfView(self):
        for i, a in enumerate(self.asteroids):
            print(
                f"parsing lines of view {int(100*i/len(self.asteroids))}%...", end="\r"
            )
    ...
    def vaporise_asteroids(self):
        ...
        detectables = list(
            filter(
                lambda x: self.do_they_see_each_other(self.station, x), self.asteroids
            )
        )
