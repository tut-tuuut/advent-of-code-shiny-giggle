...
class Nanofactory:
    ...
    def necessaryOreForXFuel(self, fuelQty):
        self.reinit()
        self.make(fuelQty, "FUEL", 0)
        return self.exctractedOre
...
def dumbPart2():
    ...
    while oreReserve > f.exctractedOre:
        ...
        fuelqty += fueltomake
        print(
            f"fuel qty : {fuelqty} extracted ore: {f.exctractedOre} ({percentage}%)",
            end="\r",
        )
    print("")
...
def part2(inputStr):
    f = Nanofactory()
    f.parseRecipes(inputStr)
    oreForOneFuel = f.necessaryOreForXFuel(1)
    print(f"this should be 13312: {oreForOneFuel}")
    oreReserve = 1000000000000
    minimumFeasibleFuel = int(oreReserve / oreForOneFuel)
    # find by dichotomy
    low = minimumFeasibleFuel
    high = minimumFeasibleFuel * 2
    pointer = int((low + high) / 2)
    print(f"lets analyze fuel qties between {low} and {high}")
    print(f"starting at {pointer}")
    while high - low > 1:
        if f.necessaryOreForXFuel(pointer) <= oreReserve:
            low = pointer
        else:
            high = pointer
        pointer = int((low + high) / 2)
    print(f"with THAT MUCH ORE you can make {low} fuel")
